...
// 리포지토리 묶음 주입받기
func NewNuboOAuthService(repos *repositories.Repository) *NuboOAuthService {
	return &NuboOAuthService{repos: repos}
}

// OAuth 계정에 프로필 이미지가 있다면 가져와 저장하기
func (s *NuboOAuthService) SaveProfileImage(userUid uint, profile string) {
...
func (s *NuboOAuthService) RegisterOAuthUser(id string, name string, profile string) uint {
...
// 리프레시 토큰을 DB에 저장해주기
func (s *NuboOAuthService) SaveRefreshToken(userUid uint, token string) {
	s.repos.Auth.SaveRefreshToken(userUid, token)
	s.repos.Auth.UpdateUserSignin(userUid)
}

// 회원 아이디(이메일)에 해당하는 고유 번호 반환
func (s *NuboOAuthService) GetUserUid(id string) uint {
